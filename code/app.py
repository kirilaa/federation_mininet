...
contract_address = Web3.toChecksumAddress('0xed929A8E6f11A83b9671B4676B8CBDE08d96968e')
...
# print("\n\tAnnounce service ------>\n")
if debug_txt == 'y' or debug_txt == 'Y':
    new_service = Federation_contract.functions.AnnounceService(_requirements= web3.toBytes(text = service_requirements), _endpoint_consumer= web3.toBytes(text = service_endpoint_consumer), _id = web3.toBytes(text = service_id)).transact({'from':coinbase})
...
if debug_txt == '1' or debug_txt == '2':
    Federation_contract.functions.PlaceBid(_id= web3.toBytes(text= service_id), _price= service_price, _endpoint= web3.toBytes(text= service_endpoint_provider)).transact({'from':service_provider_address[int(debug_txt)]})
